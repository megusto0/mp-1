...
def add_formula(doc, latex_or_text, *, number=None):
    """Формула: центрированный абзац, при наличии — номер справа."""
    p = doc.add_paragraph()
    pf = p.paragraph_format
    pf.line_spacing = 1.5
    pf.space_before = Pt(6)
...
add_formula(doc, "f(x, y) = 2·(x − 3)² + 3·(y + 2)² − 2·x·y + 4·x − 6·y,", number="2.1")
p = doc.add_paragraph()
p.paragraph_format.line_spacing = 1.5
...
add_formula(doc, "f(x, y) = 2x² + 3y² − 2xy − 8x + 6y + 30.", number="3.1")
...
add_formula(doc, "∂f/∂x = 4x − 2y − 8,", number="3.2")
add_formula(doc, "∂f/∂y = −2x + 6y + 6.", number="3.3")
...
add_formula(doc, "⎧ 4x − 2y − 8 = 0,    ⎨   −2x + 6y + 6 = 0.")
...
add_formula(doc, "x* = (1,8;  −0,4),    f(x*) = 21,6.", number="3.4")
...
add_formula(doc, "∂²f/∂x² = 4,   ∂²f/∂y² = 6,   ∂²f/∂x∂y = −2.")
add_p(doc, "Матрица Гессе:")
add_formula(doc, "H = [[4, −2], [−2, 6]].", number="3.5")
add_p(doc, "По критерию Сильвестра проверим положительную определённость:")
add_formula(doc, "Δ₁ = 4 > 0,    Δ₂ = det H = 4·6 − (−2)·(−2) = 20 > 0.")
...
add_formula(doc, "f(x) = ½·xᵀ·Q·x + bᵀ·x + c,", number="4.1")
add_p(doc, "где Q = [[4, −2], [−2, 6]], b = (−8; 6), c = 30. Градиент тогда")
add_formula(doc, "∇f(x) = Q·x + b.", number="4.2")
add_p(doc, "Итерация наискорейшего спуска и явная формула оптимального шага для квадратичной функции:")
add_formula(doc, "x_{k+1} = x_k − α_k · ∇f(x_k),    α_k = (gᵀg)/(gᵀQg).", number="4.3")
...
add_formula(doc, "d_0 = −∇f(x_0);", number="4.4")
add_formula(doc, "α_k = −(gᵀd_k)/(d_kᵀQd_k);", number="4.5")
add_formula(doc, "x_{k+1} = x_k + α_k · d_k;", number="4.6")
add_formula(doc, "β_k = ‖g_{k+1}‖² / ‖g_k‖²;", number="4.7")
add_formula(doc, "d_{k+1} = −g_{k+1} + β_k · d_k.", number="4.8")
